...
public class DatabaseDropHandler extends SQLiteOpenHelper
{
	...
	public List<DBLocations> getAllDropLocations()
	{
		...
		// Select All Query
		String selectQuery = "SELECT  DISTINCT "+KEY_DROP_FORMATTED_ADDRESS_NAME+", "+KEY_DROP_LATITUDE+", "+KEY_DROP_LONGITUDE+" FROM " + TABLE_LOCATIONS;
		//String selectQuery = "SELECT  DISTINCT "+KEY_DROP_FORMATTED_ADDRESS_NAME+" FROM " + TABLE_LOCATIONS;
		SQLiteDatabase db = this.getWritableDatabase();
		...
		if (cursor.moveToFirst()) {
			do
			{
				...
				contact.setFormattedAddress(cursor.getString(0));
